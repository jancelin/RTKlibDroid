...
#client_socket.connect(('home.complement-terre.fr', 5000))
client_socket.settimeout(10)
GPS_List = [0.0]
time.sleep(2)

status = 'status 1\r\n'
client_socket.send(status.encode('latin1'))
dict = {}
while True:
...
#          key = 'monitor port'
          if key in dict:
             ...
             print('ratio :'+dict['ratio for ar validation'])
             print('age :'+dict['age of differential (s)'])
             print('llh :'+dict['pos llh single (deg,m) rover'])
             ...
             if dict['solution status'] == 'single':
                 print('sde(m): null')
                 print('sdn(m): null')
                 print('sdu(m): null')
             elif dict['solution status'] == 'float':
                 print('sde(m):' + POS_XYZ_Float_Std_X)
                 print('sdn(m):' + POS_XYZ_Float_Std_Y)
                 print('sdu(m):' + POS_XYZ_Float_Std_Z)
             elif dict['solution status'] == 'fix':
                 print('sde(m):' + POS_XYZ_Fixed_Std_X)
                 print('sdn(m):' + POS_XYZ_Fixed_Std_Y)
                 print('sdu(m):' + POS_XYZ_Fixed_Std_Z)
             else:
                 print("no signal")
